...
def run_random_budget(env, init_states, horizon=None):
    ...
    with tqdm(total=E, desc="random", file=sys.stdout, mininterval=1.0, ncols=100) as pbar:
        for ep in range(E):
            status, _ = env.reset()
...
def main():
    ...
    pd.DataFrame(traj_rows).to_csv(os.path.join(run_dir, "trajectories.csv"), index=False)
    ...
    mean_y = np.mean([
        cum_pos_2d[ep] / max(out["total_pos"][ep], 1.0)
        for ep in range(E)
    ], axis=0)
